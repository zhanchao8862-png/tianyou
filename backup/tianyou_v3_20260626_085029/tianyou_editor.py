...
class TianyouEditor(object):
    ...
    _ROUTED_ATTRS = frozenset([
        ...
        '_info_text',
        '_item_detail_canvas',
        '_item_detail_other_vars',
        '_item_detail_type_vars',
        '_item_detail_vars',
        '_items_loaded',
        '_items_panel',
        '_items_redo_stack',
    ...
    def _build_item_detail_panel(self, parent):
        """Build scrollable item detail form panel with F0-F9 numbering."""
        ...
        self._item_detail_vars = {}

        pady_val = 2
        padx_val = (5, 2)

        def _row(parent, label_text, var_name, row, col=0, width=45, readonly=False):
            """Add a label + entry row, store var."""
            tk.Label(parent, text=label_text + ':', anchor='e', width=16).grid(
                row=row, column=col, sticky='e', padx=padx_val, pady=pady_val)
            var = tk.StringVar()
            entry = tk.Entry(parent, textvariable=var, width=width)
            entry.grid(row=row, column=col + 1, sticky='w', padx=2, pady=pady_val)
            self._item_detail_vars[var_name] = var
            return var

        def _row2(parent, label_text, var_name, row, col):
            """Add two-column label + entry."""
            tk.Label(parent, text=label_text + ':', anchor='e', width=12).grid(
                row=row, column=col, sticky='e', padx=padx_val, pady=1)
            var = tk.StringVar()
            tk.Entry(parent, textvariable=var, width=16).grid(
                row=row, column=col + 1, sticky='w', padx=2, pady=1)
            self._item_detail_vars[var_name] = var
            return var

        def _damage_row(parent, label_text, var_name, type_var_name, row, col):
            """Label + value entry + damage type Combobox in one row."""
            tk.Label(parent, text=label_text + ':', anchor='e', width=12).grid(
                row=row, column=col, sticky='e', padx=padx_val, pady=1)
            var = tk.StringVar()
            tk.Entry(parent, textvariable=var, width=8).grid(
                row=row, column=col + 1, sticky='w', padx=2, pady=1)
            self._item_detail_vars[var_name] = var
            type_var = tk.StringVar(value='cut')
            cb = ttk.Combobox(parent, textvariable=type_var, width=6,
                              values=['cut', 'pierce', 'blunt'], state='readonly')
            cb.grid(row=row, column=col + 2, sticky='w', padx=1, pady=1)
            self._item_detail_vars[type_var_name] = type_var
            return var, type_var

        # ── Section 1: Basic F0-F1 + F5 + Chinese ──
        s1 = tk.LabelFrame(f, text=u'\u57fa\u672c\u5c5e\u6027', font=('', 10, 'bold'))
        s1.pack(fill='x', padx=5, pady=3)
        _row(s1, u'F0 ID', 'id', 0)
        _row(s1, u'F1 \u540d\u79f0', 'name', 1)
        _row(s1, u'     \u6c49\u5316\u540d\u79f0', 'cn_name', 2)
        # Trace cn_name change -> write back to item_cn
        self._item_detail_vars['cn_name'].trace('w', self._on_cn_name_changed)
        _row(s1, u'F4 \u80fd\u529b (Capabilities)', 'capabilities', 3)
        _row(s1, u'F5 \u4ef7\u503c (Value)', 'value', 4)

        # ── Section 2: Meshes F2 ──
        s2 = tk.LabelFrame(f, text=u'F2 \u6a21\u578b (Meshes)', font=('', 10, 'bold'))
        s2.pack(fill='x', padx=5, pady=3)
        _row(s2, u'\u6a21\u578b\u540d\u79f0', 'mesh_name', 0)
        _row(s2, u'\u6a21\u578b\u6807\u5fd7', 'mesh_flag', 1)
        _row(s2, u'\u52a8\u4f5c (Action)', 'mesh_action', 2)

        # ── Section 3: Type flags F3 (18 checkboxes, 6 cols) ──
        s3 = tk.LabelFrame(f, text=u'F3 \u7c7b\u578b\u6807\u8bb0 (Type Flags)', font=('', 10, 'bold'))
        s3.pack(fill='x', padx=5, pady=3)
        type_flags = [
            ('itp_type_horse', u'\u9a6c horse'),
            ('itp_type_one_handed_wpn', u'\u5355\u624b\u6b66\u5668 1h'),
            ('itp_type_two_handed_wpn', u'\u53cc\u624b\u6b66\u5668 2h'),
            ('itp_type_polearm', u'\u957f\u6746 polearm'),
            ('itp_type_arrows', u'\u7bad arrows'),
            ('itp_type_bolts', u'\u5f29\u77e2 bolts'),
            ('itp_type_shield', u'\u76fe shield'),
            ('itp_type_bow', u'\u5f13 bow'),
            ('itp_type_crossbow', u'\u5f29 crossbow'),
            ('itp_type_thrown', u'\u6295\u63b7 thrown'),
            ('itp_type_goods', u'\u5546\u54c1 goods'),
            ('itp_type_head_armor', u'\u5934\u76d4 head'),
            ('itp_type_body_armor', u'\u8eab\u4f53 body'),
            ('itp_type_foot_armor', u'\u978b foot'),
            ('itp_type_hand_armor', u'\u624b\u5957 hand'),
            ('itp_type_pistol', u'\u624b\u67aa pistol'),
            ('itp_type_musket', u'\u706b\u67aa musket'),
            ('itp_type_bullets', u'\u5b50\u5f39 bullets'),
        ]
        self._item_detail_type_vars = {}
        for ti, (tf, tl) in enumerate(type_flags):
            var = tk.IntVar()
            cb = tk.Checkbutton(s3, text=tl, variable=var)
            cb.grid(row=ti // 6, column=ti % 6, sticky='w', padx=3, pady=1)
            self._item_detail_type_vars[tf] = var

        # ── Section 4: Stats F6 (dynamic fields + damage types) ──
        s4 = tk.LabelFrame(f, text=u'F6 \u5c5e\u6027 (Stats)', font=('', 10, 'bold'))
        s4.pack(fill='x', padx=5, pady=3)
        ...
        stat_entries = [
            ...
            (u'body_armor', u'\u8eab\u9632'),
            (u'leg_armor', u'\u817f\u9632'),
            (u'speed_rating', u'\u901f\u5ea6\u8bc4\u7ea7'),
            (u'horse_speed', u'\u9a6c\u901f'),
            (u'horse_maneuver', u'\u9a6c\u64cd\u63a7'),
            (u'horse_charge', u'\u9a6c\u51b2\u51fb'),
            (u'horse_scale', u'\u9a6c\u5c3a\u5bf8'),
            (u'abundance', u'\u6570\u91cf'),
            (u'armor', u'\u62a4\u7532'),
        ]
        for si, (sk, sl) in enumerate(stat_entries):
            row = si // 2
            col = (si % 2) * 3
            _row2(s4, sl, sk, row, col)

        # Damage type rows (span full width)
        dmg_row = len(stat_entries) // 2 + (1 if len(stat_entries) % 2 else 0)
        _damage_row(s4, u'\u6325\u780d\u4f24\u5bb3', 'swing_damage', 'swing_damage_type', dmg_row, 0)
        _damage_row(s4, u'\u523a\u51fb\u4f24\u5bb3', 'thrust_damage', 'thrust_damage_type', dmg_row + 1, 0)
        _damage_row(s4, u'\u51b2\u950b\u4f24\u5bb3', 'horse_charge_damage', 'horse_charge_damage_type', dmg_row + 2, 0)

        # ── Section 5: Other flags (33 checkboxes, 6 cols) ──
        s5 = tk.LabelFrame(f, text=u'\u5176\u4ed6\u6807\u8bb0 (Other Flags)', font=('', 10, 'bold'))
        s5.pack(fill='x', padx=5, pady=3)
        other_flags = [
            ('itp_merchandise', u'\u5e97\u54c1 merchandise'),
            ('itp_wooden_attack', u'\u6728\u5236\u653b wood_attk'),
            ('itp_wooden_parry', u'\u6728\u5236\u62db wood_parry'),
            ('itp_wooden_weapon', u'\u6728\u5236\u6b66 wood_wpn'),
            ('itp_cant_reload_on_horseback', u'\u9a6c\u4e0a\u7981\u88c5\u586b'),
            ('itp_two_handed', u'\u53cc\u624b 2h'),
            ('itp_primary', u'\u4e3b\u6b66\u5668 primary'),
            ('itp_secondary', u'\u526f\u6b66\u5668 secondary'),
            ('itp_covers_legs', u'\u906e\u6321\u817f\u90e8 covers_legs'),
            ('itp_doesnt_cover_hair', u'\u4e0d\u906e\u5934\u53d1 no_hair'),
            ('itp_can_penetrate_shield', u'\u53ef\u7a7f\u76fe penetrate'),
            ('itp_civilian', u'\u5e73\u6c11 civilian'),
            ('itp_next_item_as_melee', u'\u4e0b\u6b65\u8fd1\u6218 melee_next'),
            ('itp_is_pike', u'\u957f\u67aa pike'),
            ('itp_cant_use_on_horseback', u'\u9a91\u5175\u7981\u7528 no_horse'),
            ('itp_no_parry', u'\u65e0\u62db\u67b6 no_parry'),
            ('itp_default_ammo', u'\u9ed8\u8ba4\u5f39\u836f def_ammo'),
            ('itp_has_upper_stab', u'\u4e0a\u523a upper_stab'),
            ('itp_extra_penetration_flag', u'\u989d\u5916\u7a7f\u900f extra_pen'),
            ('itp_has_lower_stab', u'\u4e0b\u523a lower_stab'),
            ('itp_bonus_against_shield', u'\u7834\u76fe bonus_shield'),
            ('itp_extra_penetration', u'\u7a7f\u900f extra_pen'),
            ('itp_consumable', u'\u6d88\u8017\u54c1 consumable'),
            ('itp_couchable', u'\u5e73\u6301\u957f\u67aa couchable'),
            ('itp_crush_through', u'\u51b2\u7834 crush_through'),
            ('itp_remove_item_on_use', u'\u4f7f\u7528\u540e\u4e22\u5f03'),
            ('itp_no_pick_up_from_ground', u'\u4e0d\u53ef\u62fe\u8d70'),
            ('itp_can_knock_down', u'\u53ef\u51fb\u5012 knock_down'),
            ('itp_fit_to_head', u'\u9002\u914d\u5934\u90e8 fit_head'),
            ('itp_force_attach_left_hand', u'\u5f3a\u5236\u5de6\u624b force_L'),
            ('itp_force_attach_right_hand', u'\u5f3a\u5236\u53f3\u624b force_R'),
            ('itp_force_show_body', u'\u663e\u793a\u8eab\u4f53 show_body'),
            ('itp_force_show_left_hand', u'\u663e\u793a\u5de6\u624b show_L'),
            ('itp_force_show_right_hand', u'\u663e\u793a\u53f3\u624b show_R'),
            ('itp_no_blur', u'\u65e0\u6a21\u7cca no_blur'),
        ]
        self._item_detail_other_vars = {}
        for oi, (of, ol) in enumerate(other_flags):
            var = tk.IntVar()
            cb = tk.Checkbutton(s5, text=ol, variable=var)
            cb.grid(row=oi // 6, column=oi % 6, sticky='w', padx=3, pady=1)
            self._item_detail_other_vars[of] = var

        # ── Section 6: Modifier bits F7 ──
        s6 = tk.LabelFrame(f, text=u'F7 \u4fee\u9970\u7b26 (Modifier Bits)', font=('', 10, 'bold'))
        s6.pack(fill='x', padx=5, pady=3)
        imod_presets = [
            u'imodbits_none', u'imodbits_horse_basic', u'imodbits_cloth',
            u'imodbits_armor', u'imodbits_plate', u'imodbits_polearm',
            u'imodbits_shield', u'imodbits_sword', u'imodbits_sword_high',
            u'imodbits_axe', u'imodbits_mace', u'imodbits_pick',
            u'imodbits_bow', u'imodbits_crossbow', u'imodbits_missile',
            u'imodbits_thrown', u'imodbits_thrown_minus_heavy',
            u'imodbits_horse_good', u'imodbits_good', u'imodbits_bad',
        ]
        tk.Label(s6, text=u'Modifier:', anchor='e', width=10).grid(row=0, column=0, sticky='e', padx=(5, 0), pady=2)
        self._item_detail_vars['modifier'] = tk.StringVar()
        ttk.Combobox(s6, textvariable=self._item_detail_vars['modifier'],
                     values=imod_presets, width=30).grid(row=0, column=1, sticky='w', padx=2, pady=2)

        # ── Section 7: Triggers F8 & Factions F9 ──
        s7 = tk.LabelFrame(f, text=u'F8 \u89e6\u53d1 & F9 \u9635\u8425 (Triggers & Factions)', font=('', 10, 'bold'))
        s7.pack(fill='x', padx=5, pady=3)
        _row(s7, u'F8 Triggers', 'triggers', 0, width=60)
        _row(s7, u'F9 Factions', 'factions', 1, width=60)


    def _on_cn_name_changed(self, *args):
        """Write changed Chinese name back to item_cn."""
        cn = self._item_detail_vars.get('cn_name')
        if cn is None:
            return
        cn_val = cn.get().strip()
        # Determine full_id from id field
        iid_var = self._item_detail_vars.get('id')
        if iid_var:
            iid = iid_var.get().strip().strip('"').strip("'")
            full_id = 'itm_' + iid if iid and not iid.startswith('itm_') else iid
            if full_id and hasattr(self, 'item_cn'):
                if cn_val:
                    self.item_cn[full_id] = cn_val
    ...
    def _show_items_detail(self, idx):
        ...
        iid = (flds[0].strip('"').strip("'") if flds else '')
        full_id = 'itm_' + iid if iid and not iid.startswith('itm_') else iid
        zh = self.item_cn.get(full_id, u'(\u65e0\u6c49\u5316)') if hasattr(self, 'item_cn') else u'(\u65e0\u6c49\u5316)'
        if not zh:
            zh = u'(\u65e0\u6c49\u5316)'
        ...
        self._items_info_text.insert('end', '\n'.join(info))
        self._items_info_text.config(state='disabled')

        # Detail tab: populate form fields
        if hasattr(self, '_item_detail_vars'):
    ...
    def _populate_item_detail(self, flds):
        """Populate the detail form fields from parsed item fields."""
        while len(flds) < 10:
            flds.append('')

        # Section 1: Basic
        self._item_detail_vars['id'].set(flds[0].strip('"\u2019\u2018') if flds[0] else '')
        self._item_detail_vars['name'].set(flds[1].strip('"\u2019\u2018') if flds[1] else '')
        self._item_detail_vars['capabilities'].set(flds[4].strip() if len(flds) > 4 and flds[4] else '')
        self._item_detail_vars['value'].set(flds[5].strip() if len(flds) > 5 and flds[5] else '0')

        # Chinese name
        iid = flds[0].strip('"').strip("'") if flds[0] else ''
        full_id = 'itm_' + iid if iid and not iid.startswith('itm_') else iid
        cn_val = self.item_cn.get(full_id, '') if hasattr(self, 'item_cn') else ''
        self._item_detail_vars['cn_name'].set(cn_val)

        # Section 2: Meshes
        model_raw = flds[2] if len(flds) > 2 else '[]'
        mesh_name = ''
        mesh_flag = '0'
        mesh_action = ''
        if model_raw.startswith('[') and model_raw.endswith(']'):
            inner = model_raw[1:-1].strip()
            if inner:
                import re as _re
                m = _re.match(r'^\(\s*"([^"]*)"\s*,\s*(\d+)\s*(?:,\s*"([^"]*)")?\s*\)', inner)
                if m:
                    mesh_name = m.group(1) or ''
                    mesh_flag = m.group(2) or '0'
                    mesh_action = m.group(3) or ''
        self._item_detail_vars['mesh_name'].set(mesh_name)
        self._item_detail_vars['mesh_flag'].set(mesh_flag)
        self._item_detail_vars['mesh_action'].set(mesh_action)

        # Section 3: Type flags
        type_expr = flds[3].strip() if len(flds) > 3 and flds[3] else ''
        type_set = set(t.strip() for t in type_expr.split('|') if t.strip())
        for flag, var in self._item_detail_type_vars.items():
            var.set(1 if flag in type_set else 0)

        # Section 4: Stats
        stats_expr = flds[6].strip() if len(flds) > 6 and flds[6] else ''
        stats = {}
        if stats_expr:
            raw_stats = stats_expr
            if not raw_stats.endswith(')'):
                raw_stats += ')'
            import re as _re
            while raw_stats:
                m = _re.match(r'^(\w+)\(([^)]*)\)', raw_stats)
                if not m:
                    if raw_stats.startswith('|'):
                        raw_stats = raw_stats[1:].lstrip()
                        continue
                    break
                key = m.group(1)
                val = m.group(2)
                if key in ('swing_damage', 'thrust_damage', 'horse_charge_damage'):
                    # val like "20, cut" -> split into value + type
                    parts = val.split(',')
                    stats[key] = parts[0].strip()
                    type_key = key + '_type'
                    stats[type_key] = parts[1].strip() if len(parts) > 1 else 'cut'
                else:
                    stats[key] = val
                raw_stats = raw_stats[m.end():]
                if raw_stats.startswith('|'):
                    raw_stats = raw_stats[1:]
        for sk, var in self._item_detail_vars.items():
            if sk.startswith('swing_damage_type') or sk.startswith('thrust_damage_type') or sk.startswith('horse_charge_damage_type'):
                continue  # handled separately
        for sk in ['weight', 'difficulty', 'spd_rtng', 'weapon_length', 'max_ammo',
                    'shoot_speed', 'accuracy', 'head_armor', 'body_armor', 'leg_armor',
                    'speed_rating', 'horse_speed', 'horse_maneuver', 'horse_charge',
                    'horse_scale', 'abundance', 'armor',
                    'swing_damage', 'thrust_damage', 'horse_charge_damage']:
            if sk in self._item_detail_vars:
                self._item_detail_vars[sk].set(stats.get(sk, ''))
        # Set damage types
        for dtk in ['swing_damage_type', 'thrust_damage_type', 'horse_charge_damage_type']:
            if dtk in self._item_detail_vars:
                self._item_detail_vars[dtk].set(stats.get(dtk, 'cut'))

        # Section 6: Modifier
        self._item_detail_vars['modifier'].set(flds[7].strip() if len(flds) > 7 and flds[7] else '')

        # Section 7: Triggers & Factions
        trig_raw = flds[8] if len(flds) > 8 and flds[8] else ''
        if trig_raw and trig_raw != '[]':
            self._item_detail_vars['triggers'].set(trig_raw)
        else:
            self._item_detail_vars['triggers'].set('')
        fac_raw = flds[9] if len(flds) > 9 and flds[9] else ''
        if fac_raw and fac_raw != '[]':
            self._item_detail_vars['factions'].set(fac_raw)
        else:
            self._item_detail_vars['factions'].set('')


    def _on_cn_name_changed(self, *args):
        """When user edits cn_name field, write back to item_cn."""
        new_cn = self._item_detail_vars.get('cn_name')
        if new_cn is None:
            return
        new_val = new_cn.get().strip()
        iid = self._item_detail_vars.get('id')
        if iid:
            raw_id = iid.get().strip()
            ...
            if full_id and hasattr(self, 'item_cn'):
                ...
                # Refresh list display
                if hasattr(self, '_populate_items_list'):
                    self._populate_items_list()
                # Refresh title
                if hasattr(self, 'items_detail_title') and self._current_item_idx >= 0:
                    zh = self.item_cn.get(full_id, u'(\u65e0\u6c49\u5316)')
                    if not zh:
                        zh = u'(\u65e0\u6c49\u5316)'
